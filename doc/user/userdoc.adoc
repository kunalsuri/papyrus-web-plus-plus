...
The next image shows the home page, with no project (first run of the *Papyrus-Web* instance).

image::homepage_empty.png[title="Empty Papyrus-Web home page",1000, align="left"]

After two project creations, the home page looks to the next image. 

image::homepage_projectMenu.png[title="Papyrus-Web home page with 2 created projects",800, align="left"]
...
For further information about project creation go to xref:../faq/faq.adoc#HowTo_ProjectCreation[this section].
...
* Duplicate a project +
You can duplicate a project from the Projects Browser:
. In the Project list, go over the Project to get the *Duplicate* button,
. Click on the *Duplicate* button.

* Download a project[[HowTo_DownloadProject]] +
You can download a project as a zip archive from the Projects Browser:
. In the Project list, go over the Project to get the *Download* button,
. Click on the *Download* button, then a system dialog appears to ask you the location to save the Project. +
The section xref:../faq/faq.adoc#HowTo_ProjectCreation[How to create a new project?] explain how to start a project from a *zip* archive.

Once the project (new or existing) is chosen, the <<Workbench,workbench>> page is displayed.
...
image::workbench_presentation.png[title="Papyrus-Web workbench",1000, align="left"]
...
* _Explorers_ +
  The list of explorers available to present the given project depending on its nature.
...
Since Papyrus web 2025.10.0, representations, mainly Explorer view and diagrams, have their own local selection tools. When the user interacts with a representation (e.g. item selection in the Explorer or a node on a diagram), both the local selection of that representation and the global workbench selection are updated.

. A new toggle button is available in the _Details_ view, allowing the user to “pin” the view to the current selection and stop following global selection changes. When pin mode is disabled, the view resumes synchronization with the global selection.

[%autowidth, cols="a,a", frame=none, grid=none, role="center"]
|===
|image::unpinned_pinToggleButton.png[Selection follows global selection in Details view,300, align="center"]
|image::pinned_unpinToggleButton.png[Automatic synchronization is disabled in Details view,300, align="center"]
|===

[start=2]
. Another toggle button is available both in the diagram toolbar and in the Explorer view. It allows the user to highlight the local selection from one representation (Explorer or diagram) directly in the other.

[%autowidth, cols="a,a", frame=none, grid=none, role="center"]
|===
|image::selectToggleButtonFromDiagramToolbar.png[Highlight local selection from Diagram in Explorer,350, align="center"]
|image::selectToggleButtonFromExplorer.png[Highlight local selection from Explorer in Diagram,400, align="center"]
|===
...
The properties of this stereotype are now displayed in their own tab in the _Detail_ panel.

=== Validation panel

The _Validation_ panel shares the left area of the workbench page with the _Explorer_.
The validation view shows diagnostics of the whole project. Diagnostics are sorted by kind (e.g. ERROR, WARNING or INFO) and can be either hidden or displayed thanks to an accordion widget.

=== Views Explorer
The Views Explorer is an additional view available in the workbench that focuses on the exploration and management of representations (such as diagrams and other views) contained in the project.
A simple click on a representation opens it immediately, which can be inconvenient when the user only wants to manage the representation (for example rename it, delete it, or inspect its properties).

image::views-explorer-panel.png[title="Views Explorer panel",500, align="center"]
...
The Diagram panel has global actions grouped in a horizontal toolbar underneath the diagram tabs. This toolbar can be collapsed or expanded as needed.
...
For element displayed in compartment node with list layout, the label composed of keyword, stereotype and name is displayed on one line.
...
===== Graphical Drag&Drop
User can select a node on the diagram and drag and drop it in another container node. Only authorized containers are revealed on the diagram, forbidden targets are faded. When user try to drag and drop a node, the target container node is highlighted.
...
This behavior can also be configured later in the diagram settings.
Open the *Settings* page and enable or disable the option *Enable confirmation dialogs* in the *General Settings* section.

image::settings-confirmation-dialogs.png[Confirmation dialogs setting,600, align="center"]
...
Since Sirius web 2025.6.0, diagram elements appearance can be customized by using the contextual palette:

image::appearance.png[Node appearance,400, align="center"]

[NOTE]
====
This is not yet implemented for nodes specific to Papyrus Web (The Cuboid node for instance):

image::appearanceOnCuboid.png[Cuboid appearance,400, align="center"]
====
==== Symbols

The goal of the symbol support in Papyrus web is to give users access to some custom representations. They can use their own representation, i.e. their own svg and png files, in their diagrams, so the model can be easily read and understood.
...
Once symbols are added on the diagram, it is possible to switch from the default representation with both symbols and default compartment, to symbol only representation or no symbol representation using show hide tools available from the diagram background.
...
====== Show / hide all other compartments
...
- On a UML element, if the feature multiplicity is _[0..1]_ or _[1..1]_ or _[1..*]_, a _Pin_ is created at the same time as the element creation.
...
The Profile diagram provide a way to extend a UML model. They are based on additional stereotypes (identified as classes with the «stereotype» stereotype) and labeled values that are applied to UML element thanks to the _extension_ relation.
...
===== Specific Features on Semantic Drag&Drop
- The semantic drag and drop of an _ElementImport_ with a UML metaclass as its _imported element_ on the Profile diagram produces a Node with the name of the UML metaclass as its label. Note that it is not possible to drag and drop _ElementImport_ with no _imported element_ or with an _imported element_ that is not a UML metaclass.
...
All those concepts manage a couple of lists of `strings` which are weakly connected via their index. The first list, called '[.code]``languages``' contains the name of languages, such as `"JAVA"` or `pass:c["C++"]`. The second one contains the body expression expressed in the language of the same rank in the list. These lists could not be edited in Papyrus UI as separate ordinary lists of text fields. This is the reason why a new custom widget has been developed.
...
If a displayExpression is specified in the PrimitiveList widget, it is evaluated for each candidate.
...
The primitive list has also the capability to reorder its items. This is an optional capability that is controlled by the presence of an expression inside the view model of the widget. If such an expression is provided, a reorder icon appears on the left of the list title as displayed below:
 ...
 * The reorder icon to sort the reference values (optional: only for multivalued references). +
...
Whereas in a multivalued reference, the dialog allows the user to manage the list of reference values.
...
In case of a multivalued reference, the new element is added at the end of the value list.
...
For a mono-valued reference the selected value replace the existing one, whereas it is added to the value list in case of multivalued.
...
In most case, the currently edited reference is an _eOpposite_ of the reference that is currently containing the displayed element.
...
This is mostly caused by the use of reference type redefinition in the _UML_ metamodel (look for "redefine" _EAnnotations_).
...
This information corresponds to what is entered in the dialog each time the profile is published.
